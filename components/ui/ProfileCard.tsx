...
const ProfileCardComponent = ({
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      ...
      const anyMotion = (window as unknown as Record<string, unknown>).DeviceMotionEvent;
      if (anyMotion && typeof (anyMotion as { requestPermission?: () => Promise<string> }).requestPermission === 'function') {
        (anyMotion as { requestPermission: () => Promise<string> })
          .requestPermission()
          .then(state => {
            if (state === 'granted') {
  ...
  const shineStyle: React.CSSProperties = {
    ...
    filter: 'brightness(1) contrast(1.5) saturate(0.6) opacity(0.8)',
    ...
    backgroundImage: [
      'repeating-linear-gradient(0deg,var(--sunpillar-clr-1) calc(var(--space,5%) * 1),var(--sunpillar-clr-2) calc(var(--space,5%) * 2),var(--sunpillar-clr-3) calc(var(--space,5%) * 3),var(--sunpillar-clr-4) calc(var(--space,5%) * 4),var(--sunpillar-clr-5) calc(var(--space,5%) * 5),var(--sunpillar-clr-6) calc(var(--space,5%) * 6),var(--sunpillar-clr-1) calc(var(--space,5%) * 7))',
      'repeating-linear-gradient(var(--angle,-45deg),#0e152e 0%,hsl(180,10%,60%) 3.8%,hsl(180,29%,66%) 4.5%,hsl(180,10%,60%) 5.2%,#0e152e 10%,#0e152e 12%)',
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col items-start gap-1.5">
                      <div className="text-sm font-medium text-black leading-none">@{handle}</div>
                      <div className="text-sm text-black/60 leading-none">{status}</div>
                    </div>
                  </div>
                  <button
                    className="border border-black/20 rounded-lg px-4 py-3 text-xs font-semibold text-black/80 cursor-pointer backdrop-blur-[10px] transition-all duration-200 ease-out hover:border-black/50 hover:-translate-y-px"
              ...
              <div className="w-full absolute flex flex-col" style={{ top: '3em' }}>
                  <h3
                    ...
                    style={{
                      ...
                      backgroundImage: 'linear-gradient(to bottom, #e2e8f0, #10B981)',
                  ...
                  <p
                    ...
                    style={{
                      ...
                      fontSize: '16px',
                      margin: '0 auto',
                      backgroundImage: 'linear-gradient(to bottom, #94a3b8, #059669)',
                      backgroundSize: '1em 1.5em',
                      WebkitTextFillColor: 'transparent',
                      backgroundClip: 'text',
                      WebkitBackgroundClip: 'text'
