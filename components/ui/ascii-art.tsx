...
export const AsciiArt: React.FC<AsciiArtProps> = ({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const visualAspect = cols / rows;
      const posX = imagePositionX;
      const posY = imagePositionY;
      ...
      if (objectFit !== "contain") {
        ...
        let dstX = 0, dstY = 0, dstW = cols, dstH = rows;

        const lostLeft = Math.max(0, -srcX);
        const lostRight = Math.max(0, srcX + srcW - imgWidth);
        const lostTop = Math.max(0, -srcY);
        const lostBottom = Math.max(0, srcY + srcH - imgHeight);

        if (lostLeft + lostRight < srcW && lostTop + lostBottom < srcH) {
          srcX = Math.max(0, srcX);
          srcY = Math.max(0, srcY);
          srcW = srcW - lostLeft - lostRight;
          srcH = srcH - lostTop - lostBottom;

          dstX = Math.round((lostLeft / (srcW + lostLeft + lostRight)) * dstW);
          dstY = Math.round((lostTop / (srcH + lostTop + lostBottom)) * dstH);
          dstW = Math.round((srcW / (srcW + lostLeft + lostRight)) * dstW);
          dstH = Math.round((srcH / (srcH + lostTop + lostBottom)) * dstH);
